import gleam/erlang/process
import gleam/http.{Get, Post}
import gleam/json
import player
import players
import router_utils.{allow_cors, json_response, json_response_obj}
import wisp.{type Request, type Response}

pub fn handle(
  players_actor: process.Subject(players.Message),
  req: Request,
) -> Response {
  use req <- allow_cors(req)

  case req.method, wisp.path_segments(req) {
    Get, [] -> json_response(200, [])
    Get, ["ping"] -> json_response(200, [])

    Post, ["register"] -> {
      router_utils.on_json(req, player.registration_request_decoder, fn(rr) {
        case players.try_register(players_actor, rr.username) {
          Ok(lir) -> json_response_obj(200, player.log_in_response_to_json(lir))
          Error(err) ->
            json_response_obj(
              409,
              player.registration_failed_response_to_json(err),
            )
        }
      })
    }

    Post, ["log_in"] -> {
      router_utils.on_json(req, player.log_in_request_decoder, fn(lir) {
        case players.try_login(players_actor, lir.jwt) {
          Ok(lir) -> json_response_obj(200, player.log_in_response_to_json(lir))
          Error(err) ->
            json_response_obj(400, player.log_in_failed_response_to_json(err))
        }
      })
    }

    _, _ ->
      json_response(404, [
        #("msg", json.string("404: Not Found")),
      ])
  }
}
// TODO handle 5xx with json too
